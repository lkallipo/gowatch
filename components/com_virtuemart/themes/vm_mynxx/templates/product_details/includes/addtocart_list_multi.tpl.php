...
<div class="vmCartDetails<?php echo $cls_suffix ?>">
    ...
    <div class="vmCartChildHeading<?php echo $cls_suffix ?>">
        ...
        if( !empty( $headings )) {
	        ...
	        <?php } ?>
			<span style="float: right; width: 95px;">&nbsp;</span>
	        <span style="float: right;width: 115px; text-align: center;"><?php echo $VM_LANG->_('PHPSHOP_CART_QUANTITY') ?></span>
        ...
        <form action="<?php echo $mm_action_url ?>index.php" method="post" name="addtocart" id="addtocart<?php echo $product['product_id'] ?>" class="addtocart_form">
            <div class="vmCartChildElement<?php echo $cls_suffix ?>">
                ...
                if (USE_AS_CATALOGUE != '1'  && $product_price != "" && !stristr( $product_price, $VM_LANG->_('PHPSHOP_PRODUCT_CALL'))) { 
                	...
                	?>
                    <span class="vmChildDetail<?php echo $cls_suffix ?>" style="float: right;text-align: right;margin-top: 10px;">
                ...
                if (USE_AS_CATALOGUE != '1' ) { ?>
                    <span class="vmChildDetail<?php echo $cls_suffix ?>" style="float: right; width: 115px;"><span class="vmMultiChildQuantityBox"><?php echo $product['quantity_box'] ?></span></span>
                <?php } 
                // Output Price 
                if( $_SESSION['auth']['show_prices'] && _SHOW_PRICES) {  ?>           
                    <span class="vmChildDetail<?php echo $cls_suffix ?>" style="float: right;width: 12%;">
                    <?php
