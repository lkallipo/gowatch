...
if (USE_AS_CATALOGUE != '1' && $product_price != "" && !stristr( $product_price, $VM_LANG->_('PHPSHOP_PRODUCT_CALL') )) {
    ...
    <div style="<?php if ($children == "multi") { ?>float: right; <?php } ?>margin-right: 15px;"><div class="vmQuanityFormSurround"><div class="details-cart-form"><div class="cart-quantity-box"><?php 
    if ($children == "drop") { 
    ...
    ?></div></div></div>
    <div class="vmQuantityAddCart"><input type="submit" class="<?php echo $button_cls ?>" value="<?php echo $button_lbl ?>" title="<?php echo $button_lbl ?>" /></div>
    </div>
    <?php  } ?>    
