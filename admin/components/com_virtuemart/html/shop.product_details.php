...
if ($db->f("user_id") < 1 ){
  ...
  $Sql = "INSERT INTO #__{vm}_favorites SET product_id='$product_id', user_id='$my->id', fav_date=NOW()";
  $db->query($Sql);
  $addtofavorites = JText::_('VM_FAVORITE_ADDED')."<br \\><br \\> <a href=\"index.php?page=account.favorite_products\">".JText::_('VM_ALL_FAVORITE_PRODUCTS')."</a>";
} else {
$url= 
"?page=shop.product_details&amp;flypage=$flypage&amp;product_id=$product_id&category_id=$category_id&amp;option=com_virtuemart";
   $user_id=$auth["user_id"];
   if ( $user_id == 0 ){
      $addtofavorites =  JText::_('VM_FAVORITE_NOLOGIN');}
   
if ( $user_id > 0 ){
      $addtofavorites = "
	  <link href=\"../administrator/components/com_favorites/template.css\" rel=\"stylesheet\" type=\"text/css\" />\n
<div class=\"addtofavorites\">
        <form method=\"post\" action=\"$url\" name=\"addtofavorites\" id=\"addtofavorites\">
<input type=\"submit\" class=\"addtofav_button\" style=\"font-size:9px\" value=\"".JText::_('VM_ADD_TO_FAVORITES')."\" name=\"addtofavorites\" title=\"\" />
...
if ($db->f("user_id") > 1 ){ $addtofavorites = JText::_('VM_FAVORITE_EXIST')."<br \\><br \\> <a href=\"index.php?page=account.favorite_products\">".JText::_('VM_ALL_FAVORITE_PRODUCTS')."</a>";}

/* ADD TO FAVORITES TEMPLATE **/
$tpl->set( "addtofavorites", $addtofavorites );
...
$tpl->set( "product_rating", $product_rating );// cleverweb: added to make rating available to product details page
