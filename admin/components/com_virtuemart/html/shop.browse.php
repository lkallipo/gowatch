...
elseif( $num_rows == 1 && ( !empty($keyword) || !empty($keyword1) ) ) {
	...
	if ( $num_rows > 1 && @$_REQUEST['output'] != "pdf") {
		if ( $num_rows > 5 ) { // simplified logic
			...
			// cleverweb: cherry picker mod for search in prices
	...
	while ($db_browse->next_record()) {
		...
		$db->next_record();
		if ($db->f("user_id") < 1 ){
		if(!empty($_POST) && $product_id == $favorite_id) {
		...
		$url_fav= 
		"index.php?page=shop.browse&amp;flypage=$flypage&amp;category_id=$category_id&amp;option=com_virtuemart&amp;favorite_id=$product_id";
		...
		<form method=\"post\" action=\"$url_fav\" name=\"addtofavorites\" id=\"addtofavorites\">
		...
		if ($db->f("user_id") > 1 ){ $addtofavorites = JText::_('VM_FAVORITE_EXIST');}
