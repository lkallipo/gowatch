	...
	function list_product_type($product_id) {
		...
		while ($dbag->next_record()) { // Show all Product Type
			...
			if ($dbag->f("product_type_flypage")) {
				...
				if (file_exists($flypage_file)) {
					if(empty($product_type[$product_type_name])) {
			...
			while ($dba->next_record()) {
				...
				$product_type['features'."_".$parameter_name."_label"] = $dba->f("parameter_label");
				$product_type['features'."_".$parameter_name."_desc"] = $parameter_description;
                $product_type['features'."_".$parameter_name."_tooltip"] = $html;
                $product_type['features'."_".$parameter_name] = $dbp->f($dba->f("parameter_name"))." ".$dba->f("parameter_unit");
			}
		}
		return $product_type;
}
